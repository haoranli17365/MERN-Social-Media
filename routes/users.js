...
router.put('/:id', async (req, res) =>{
    if(req.body.userId === req.params.id || req.body.isAdmin) {
        ...
        return res.status(403).json("Update invalid.");
    }
});
...
router.delete('/:id', async (req, res) =>{
    if(req.body.userId === req.params.id || req.body.isAdmin) {
        ...
        return res.status(403).json("Delete invalid.");
    }
});
...
router.put('/:id/follow', async (req, res) =>{
    ...
    if (req.body.userId !== req.params.id) {
        try{
            ...
            // if already follow the target user.
            if (!targetUser.followers.includes(req.body.userId)){
                ...
                res.status(200).json("Followed Sucessful.")
            }else{
    ...
    if (req.body.userId !== req.params.id) {
        try{
            ...
            if (targetUser.followers.includes(req.body.userId)){
                ...
                res.status(200).json("Unfollowed Sucessful.")
            }else{
